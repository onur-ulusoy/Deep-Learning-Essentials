...
class ModelTester:
    def __init__(self, input_size=2, hidden1_size=8, hidden2_size=4, output_size=1, draw_numbers=True):
    ...
    def run_test(self, seed=7, num_points=40, noise=0.2, revolutions=4):
        ...
        self.plot_predictions(X_test, y_pred_class, y_pred, draw_numbers=self.draw_numbers)
    
    # Function to plot test points with their predicted classes and optional labels
    def plot_predictions(self, X, y_pred_class, y_pred, draw_numbers):
...
class ModelTesterTorch:
    def __init__(self, input_size=2, hidden1_size=18, hidden2_size=4, output_size=1, draw_numbers=True):
        self.input_size = input_size
        self.hidden1_size = hidden1_size
        self.hidden2_size = hidden2_size
        self.output_size = output_size
    ...
    def run_test(self, seed=7, num_points=40, noise=0.2, revolutions=4):
        ...
        # Perform a forward pass using the loaded model
        y_pred = self.network.forward(torch.tensor(X_test, dtype=torch.float32))
